...
for i in enumerate(reactValue): 
    ...
    dx, dy, dz =\
       eval(interValue[i[0]][31:38]) - eval(reactValue[i[0]][31:38]),\
       eval(interValue[i[0]][38:46]) - eval(reactValue[i[0]][38:46]),\
       eval(interValue[i[0]][46:54]) - eval(reactValue[i[0]][46:54])
    #print dx, dy, dz
...
for i in range(n): 
    ...
    for atom in range(len(reactValue)): 
        xi = eval(reactValue[atom][31:38]) + diffFrac[atom][0]*i
        yi = eval(reactValue[atom][38:46]) + diffFrac[atom][1]*i
        zi = eval(reactValue[atom][46:54]) + diffFrac[atom][2]*i
        set.append('%7.3f %7.3f %7.3f' % (xi, yi, zi))
...
def makeSeparateFiles(name):
    for step in range(1, len(interpolation)+1):
        ...
        writeString = 'charges\n\n\n'
        for atom in range(len(reactValue)):
            writeString += reactValue[atom][:31]\
