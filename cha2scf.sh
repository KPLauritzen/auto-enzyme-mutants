...
# 1SCF keyword line and two emply lines.
echo charge=$ch mozyme cutoff=3 1scf eps=78.4 >> tmp-$name.mop
echo >> tmp-$name.mop
echo >> tmp-$name.mop
...
mv tmp-$name.mop 1-3-scf-$name.mop
tail -n -$((lenTot-3)) ${1/out/mop} >> 1-3-scf-$name.mop

# Remove whitespace between 'charge=' and '-3'.
vi -c "1,1s/charge= /charge=/g" -c "wq" 1-3-scf-$name.mop
